...
struct LightningView: View {
    ...
    var body: some View {
        ZStack {
            ...
            .onReceive(timer) { _ in
                if !pauseAnimating {
                    if time == 0 {
                        // Start a new main path from the top middle of the screen
    ...
    func addBranch(from startPoint: CGPoint, mainBoltPoint: CGPoint) {
        ...
        let direction = (startPoint.x < mainBoltPoint.x) ? -1 : 1  // Determine direction based on the starting point relative to the main bolt
        
        for _ in 0..<branchSteps {
            let branchX = branchPoint.x + CGFloat.random(in: 5...30) * CGFloat(direction)  // Maintain direction for the branch
            let branchY = branchPoint.y + CGFloat.random(in: 10...20)
            branchPoint = CGPoint(x: branchX, y: branchY)
            newPath.addLine(to: branchPoint)
        }
        
        branches.append(newPath)  // Append the new branch path to the paths array
    }
}
